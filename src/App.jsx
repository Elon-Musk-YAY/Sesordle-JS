...
function getConvertedMinimumWindowSize() {
  const minimumWindowSize = new LogicalSize(830, 740);
  const devicePixelRatio = window.devicePixelRatio || 1;
  const convertedSize = {};
  convertedSize.width = Math.ceil(minimumWindowSize.width * devicePixelRatio);
  convertedSize.height = Math.ceil(minimumWindowSize.height * devicePixelRatio);

  return convertedSize;
}

const windowSize = window.localStorage.getItem("windowSize")
  ? JSON.parse(window.localStorage.getItem("windowSize"))
  : { width: 0, height: 0, type: "Physical" };
if (
  windowSize.width < getConvertedMinimumWindowSize().width ||
  windowSize.height < getConvertedMinimumWindowSize().height
) {
  appWindow.setSize(
    new PhysicalSize(
      getConvertedMinimumWindowSize().width,
      getConvertedMinimumWindowSize().height
    )
  );
} else {
  appWindow.setSize(new PhysicalSize(windowSize.width, windowSize.height));
}

const windowPos = window.localStorage.getItem("windowPosition")
  ? JSON.parse(window.localStorage.getItem("windowPosition"))
  : { x: 0, y: 0, type: "Physical" };

appWindow.setPosition(new PhysicalPosition(windowPos.x, windowPos.y));
appWindow.show();

const u1 = appWindow.onResized(({ payload: size }) => {
  if (!loaded) {
    return;
  }
  window.localStorage.setItem(
    "windowSize",
    JSON.stringify(new PhysicalSize(size.width, size.height))
  );
});

const u2 = appWindow.onMoved(({ payload: position }) => {
  if (!loaded) {
    return;
  }
  console.log("Window moved", position);
  window.localStorage.setItem("windowPosition", JSON.stringify(position));
});

// check when page is reloaaded or closed
window.addEventListener("beforeunload", function () {
  u1();
  u2();
});

document.addEventListener("tauri://menu/quit", () => {
  console.log("quit");
});
  ...
  React.useEffect(() => {
    ...
    function importData() {
      ...
      if (gameData.played) {
        ...
        if (length >= 2) {
          ...
          for (let i = 6; i < 12; i++) {
            ...
            guess_grid.children[i].addEventListener(
              ...
              () => {
                guess_grid.children[i].classList.remove("fast");
                guess_grid.children[i].dataset.state = calculateResult(
                  i - 6,
                  g
                );
                guess_grid.children[i].style.transition =
        ...
        guess_grid.children[length * 6 - 1].addEventListener(
          "transitionend",
          () => {
            setTimeout(() => {
              const tiles = Array.from(guess_grid.children).slice(
                (length - 1) * 6,
                length * 6
              );
    ...
    function updateStats() {
      ...
      if (win) {
        distrib_div.children[guess_count - 2].style.color =
          "hsl(114, 42%, 59%)";
      }
    }
    ...
    function getGuessTiles() {
      return [
        guess_grid.children[(guess_count - 1) * 6],
        guess_grid.children[(guess_count - 1) * 6 + 1],
        guess_grid.children[(guess_count - 1) * 6 + 2],
        guess_grid.children[(guess_count - 1) * 6 + 3],
        guess_grid.children[(guess_count - 1) * 6 + 4],
        guess_grid.children[(guess_count - 1) * 6 + 5],
      ];
    }
  ...
  return (
    <div>
      <div id="title-bar" data-tauri-drag-region>
        <div id="main">Sesordle</div>
      </div>
      <div id="top">
        <h1 id="title">Sesordle</h1>
      </div>
      <div id="stats-div">
        <button id="stats-button" tabIndex={-1}>
          STATS
        </button>
        {/* <button id="wipe" tabindex="-1" style="dsip":>WIPE SAVE</button> */}
      </div>
      <div id="stats_screen" style={{ display: "none" }}>
        <div id="s2">
          <div id="stats_top">STATISTICS</div>
          <div id="statistics">
            <div id="stat-container-played">
              <div id="stat" />
              <div id="label">Played</div>
            </div>
            <div id="stat-container-win-per">
              <div id="stat" />
              <div id="label">Win %</div>
            </div>
            <div id="stat-container-cur-streak">
              <div id="stat" />
              <div id="label">Current Streak</div>
            </div>
            <div id="stat-container-max-streak">
              <div id="stat" />
              <div id="label">Max Streak</div>
            </div>
          </div>
          <div id="distrib">
            <div id="distrib_top">GUESS DISTRIBUTION</div>
            <div id="distribs">
              <div id="d-1">1: 0</div>
              <div id="d-2">2: 0</div>
              <div id="d-3">3: 0</div>
          ...
          <div id="foot">
            ...
            <div id="next">
              Next Sesordle:
              <div id="nex">00:00:00</div>
            </div>
          </div>
        </div>
      </div>
      <div className="alert-container" data-alert-container></div>
      <div data-guess-grid className="guess_container">
        <div className="tile" />
        <div className="tile" />
        <div className="tile" />
        <div className="tile" />
      ...
      <h2 id="letter-bank-text">LETTER BANK</h2>
      <div data-keyboard className="keyboard" tabIndex={-1}>
        <button className="key" tabIndex={-1} data-key="A">
          A
        </button>
        <button className="key" tabIndex={-1} data-key="B">
          B
        </button>
        <button className="key" tabIndex={-1} data-key="C">
          C
        </button>
        <button className="key" tabIndex={-1} data-key="D">
          D
        </button>
        <button className="key" tabIndex={-1} data-key="F">
          F
        </button>
        <button className="key" tabIndex={-1} data-key="E">
          E
        </button>
        <button className="key" tabIndex={-1} data-key="G">
          G
        </button>
        <button className="key" tabIndex={-1} data-key="H">
          H
        </button>
        <button className="key" tabIndex={-1} data-key="I">
          I
        </button>
        <button className="key" tabIndex={-1} data-key="J">
          J
        </button>
        <button className="key" tabIndex={-1} data-key="K">
          K
        </button>
        <button className="key" tabIndex={-1} data-key="L">
          L
        </button>
        <button className="key" tabIndex={-1} data-key="M">
          M
        </button>
        <button className="key" tabIndex={-1} data-key="N">
          N
        </button>
        <button className="key" tabIndex={-1} data-key="O">
          O
        </button>
        <button className="key" tabIndex={-1} data-key="P">
          P
        </button>
        <button className="key" tabIndex={-1} data-key="Q">
          Q
        </button>
        <button className="key" tabIndex={-1} data-key="R">
          R
        </button>
        <button className="key" tabIndex={-1} data-key="S">
          S
        </button>
        <button className="key" tabIndex={-1} data-key="T">
          T
        </button>
        <button className="key" tabIndex={-1} data-key="U">
          U
        </button>
        <button className="key" tabIndex={-1} data-key="V">
          V
        </button>
        <button className="key" tabIndex={-1} data-key="W">
          W
        </button>
        <button className="key" tabIndex={-1} data-key="X">
          X
        </button>
        <button className="key" tabIndex={-1} data-key="Y">
          Y
        </button>
        <button className="key" tabIndex={-1} data-key="Z">
          Z
        </button>
      </div>
    </div>
